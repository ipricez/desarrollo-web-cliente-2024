/**
 * Crear una página web sencilla con una lista de elementos que se pueden añadir o eliminar.
 * Crea un script que almacene la lista en el almacenamiento local del navegador cada vez que se
 * agrega o elimina un elemento.
 * Crea otro script que recupere la lista desde el almacenamiento local y lo muestre en la página web cada vez que se carga la página.
 * Añade en esa página el botón BORRAR que borre los datos almacenados en el local storage.
 */
window.onload = function(){
    
}

 function verLista(){
    let i = 0;
    while(localStorage.key(i) != null){
        
    } 
}